...
- Claude Code: `claude/hooks/guard.sh`'s git global-flag tolerance (`-C`, `-c`, etc.) doesn't match option values with shell-quoted spaces, e.g. `git -c "user.name=John Doe" push` — this lets the push/reset --hard/checkout ./restore ./clean check silently not fire. Narrow and requires a deliberately quoted value before a destructive subcommand; low priority. Fix would add a quoted-value alternative like `-c "[^"]*"` to the `GIT_GLOBAL_OPT` fragment.
